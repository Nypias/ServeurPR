...
class Trajectoire :
    ...
    # send back 1 if collision 0 if the player lose the game
        ...
        if ( distance_from_raquette_center ) < 30*30:
            ...
            print str(nb_player) + " looser"
            self.joueurs[nb_player].perdre()
            self.delay = reactor.callLater(0.7, self.genererTrajectoire, (50,50), 0)
    ...
    def multi_time_calculation(self, nb_player):
        ...
        print "Arrivé dans " + str(time) + " au positionCollision = " + str(pointCollision) + " joueur n" + str(nb_player) + " prepare toi!"
        reactor.callLater(time, self.multi_collision, nb_player)
